...
public class Ticket extends TitreTransport implements Serializable {
    ...
    public boolean estValide(LocalDate dateUtilisation) throws TitreNonValideException {
        // A ticket is only valid on the day of its purchase
        if (dateAchat.isEqual(dateUtilisation)) {
            return true;
        } else {
            throw new TitreNonValideException("Ticket numéro " + id + " expiré - valable uniquement le : " + dateAchat); //throw an exception if the ticket is not valid
        }
    }
}